...
class RequestedServicesRemoteDataSource
  ...
  Future<ServiceRequestModel> getRequestDetails(int id) async {
    ...
    final token = await storage.read(key: 'token');
    final res =
        await api.get('${Endpoints.requestedServices}/$id', options: Options(headers: {'Authorization': 'Bearer $token'}));
  ...
  Future<void> deleteRequest(int id) async {
    final storage = sl<FlutterSecureStorage>();
    final token = await storage.read(key: 'token');
    await api.delete('${Endpoints.requestedServices}/$id', options: Options(headers: {'Authorization': 'Bearer $token'}));
  }
